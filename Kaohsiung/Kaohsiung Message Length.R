...
HanMess<- data.frame(Hanall2018$Date , nchar(Hanall2018$Message), Hanall20187)
ChenMess<- data.frame(Chen2018$Date, nchar(Chen2018$Message), Chen2018$Page_Name)
colnames(HanMess) <- c("time","Message_Length","name" )
colnames(ChenMess) <- c("time","Message_Length","name" )

combinedMess <- rbind.fill(HanMess, ChenMess)

ggplot(combinedMess) + geom_line(aes(x= time, y= Message_Length ,color = name) ) + labs(title="Kaohsiung")
