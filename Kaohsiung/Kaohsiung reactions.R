...
HanRC<- data.frame(Hanall2018$Date , Hanall2018$All_Reaction_Count, Hanall20187)
ChenRC<- data.frame(Chen2018$Date, Chen2018$All_Reaction_Count, Chen2018$Page_Name)
colnames(HanRC) <- c("time","reactions","name" )
colnames(ChenRC) <- c("time","reactions","name" )

combinedRC <- rbind.fill(HanRC, ChenRC)


ggplot(combinedRC) + geom_line(aes(x= time, y= reactions ,color = name) ) + labs(title="Kaohsiung")
