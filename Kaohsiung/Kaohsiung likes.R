...
bb<- data.frame(Hanall2018$Date , Hanall2018$LIKE_COUNT, Hanall20187)
aa<- data.frame(Chen2018$Date, Chen2018$LIKE_COUNT, Chen2018$Page_Name)
colnames(bb) <- c("time","likes","name" )
colnames(aa) <- c("time","likes","name" )

combined <- rbind.fill(aa, bb)


ggplot(combined) + geom_line(aes(x= time, y= likes ,color = name) ) + labs(title="Kaohsiung")
